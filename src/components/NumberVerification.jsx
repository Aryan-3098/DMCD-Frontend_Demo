...
import { Box, Input, Button, FormControl, FormErrorMessage, useToast, PinInput, PinInputField } from '@chakra-ui/react';
import { BACKEND_ENDPOINT } from '../constants';

const NumberVerification = ({ onPhoneNumberChange }) => {
    ...
    const editPhoneNumber = () => {
        setFormState({ ...formState, phoneEditable: true, showOtpInput: false })
    }
    ...
    const generateOtp = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                setFormState({ ...formState, showOtpInput: true, resendDisabled: true, phoneNumberError: '', phoneEditable: true });
                setTimeout(() => setFormState({ ...formState, resendDisabled: false }), 180000);
            } else {
                toast({
                    title: 'Error',
    ...
    const verifyOtp = async () => {
        try {
            const response = await axios.post(`${BACKEND_ENDPOINT}verifyOTP`, { code: otp });
            if (response.status === 200) {
        ...
        <Box id="numberverification">
            <FormControl isInvalid={formState.phoneNumberError}>
                <Input
                    ...
                    readOnly={!formState.phoneEditable}
                />
